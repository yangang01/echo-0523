...
test("progresses from enter through presentation to a ready idle schedule", () => {
  ...
  expect(state).toMatchObject({ phase: "ready", autoAdvanceAt: 6100 + READY_IDLE_MS, idleRemainingMs: READY_IDLE_MS });
});
...
test("ignores idle expiry while paused, before schedule, or outside ready", () => {
  ...
  expect(reduceDirector(state, { type: "IDLE_EXPIRED", now: state.autoAdvanceAt! })).toMatchObject({ phase: "exit", advanceToken: 1 });
});
