...
export function ScenePanel({ scene, children, fragments, unlocked, activeId, onSelect, onReveal, onComplete, onReadingChange }: Props) {
  ...
  return (
    <section className={`scene-panel scene-${scene}`} aria-labelledby={`scene-${scene}-title`}>
      ...
      <EchoTranscript fragments={fragments} unlocked={unlocked} activeId={activeId} onSelect={onSelect} onReveal={onReveal} onComplete={onComplete} onReadingChange={onReadingChange} />
